...
function PglyAsync ( this: IPglyAsync, options: IPglyAsyncOptions )
{
	...
	options.responseContainer = options.responseContainer || `${options.container}--response`;
		...
		.forEach( ($el: Element) => {
			...
			$button.addEventListener('click', e => {
				...
				if ( this.options.form && data.form )
				{
					...
					const form: Array<IPglyAsyncFormField> = this.options.form[data.form] || [];
					let error = false;

					form.forEach((i: IPglyAsyncFormField) => {
						const $el = document.getElementById(i.id) as HTMLInputElement;
					...
					if ( error )
					{
						...
						this._consoleError('Cannot send, invalid fields detected', data.form);
